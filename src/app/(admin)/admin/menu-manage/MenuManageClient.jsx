...
export default function MenuManageClient({ initialItems }) {
  ...
  return (
    <div className="p-8 bg-slate-50 min-h-screen">
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.id} className="border-b border-gray-50 hover:bg-slate-50/50 transition-colors group">
                ...
                <td className="py-4 px-6 text-center">
                  {item.isAvailable ? (
                    <span className="text-green-500 font-bold text-sm flex items-center justify-center gap-1.5">
      ...
      {isFormOpen && (
        <MenuItemForm item={editingItem} onCancel={closeForm} onSubmit={handleSubmit} />
      )}
    </div>
  );
}
