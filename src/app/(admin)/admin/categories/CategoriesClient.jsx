...
export default function CategoriesClient({ initialCategories }) {
  ...
  const handleDelete = async (id, catName) => {
    if (!confirm(`Do you want to delete "${catName}"?`)) return;
    try {
        ...
        <div className="md:col-span-2">
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {categories.map((cat) => (
              <div
                key={cat.id}
                className="bg-white p-4 rounded-3xl shadow-sm border border-gray-100 flex flex-col items-center justify-center relative group"
              >
                <button
